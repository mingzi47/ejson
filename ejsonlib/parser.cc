...
Parser::Parser()
    : tokens(nullptr), json_data(), pos(0)
{
}

Parser::Parser(std::string data)
    : tokens(data), json_data(), pos(0)
{
    Parser::parse();
}
        ...
        switch (tokens[pos])
        {
            case '\\' : 
            {
                ...
                switch (tokens[pos])
                {
                    ...
                    case 'u' : 
                    {
                         for (int i = 0; i < 4; i++, pos++) data.push_back(tokens[pos]);                   
                         break;
                    }
                    default :
...
Json Parser::parse_value()
{
    ...
    switch (tokens[pos])
    {
        ...
        case 't' : return parse_bool(true); 
        case 'f' : return parse_bool(false); 
...
Json Parser::parse()
{
    ...
    if (tokens.size() > pos)
    {
        std::logic_error("parse is failed ! ");
    }
