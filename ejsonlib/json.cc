...
Json::Json(Type type) : _type{ type } {
    switch(_type) {
        case Type::JSON_NULL: _value = nullptr; break;
        case Type::JSON_BOOL: _value = false; break;
        case Type::JSON_INT: _value = 0; break;
        case Type::JSON_DOUBLE: _value = 0.0; break;
        case Type::JSON_STRING: _value = std::make_unique<string>(); break;
        case Type::JSON_ARRAY: _value = std::make_unique<array>(); break;
        case Type::JSON_OBJECT: _value = std::make_unique<object>(); break;
    }
}

Json::Json(std::nullptr_t value)
    : _type{ Type::JSON_NULL }, _value{ value } {}

Json::Json(const bool value)
    : _type{ Type::JSON_BOOL }, _value{ value } {}

Json::Json(const int value)
    : _type{ Type::JSON_INT }, _value{ value } {}

Json::Json(const double value)
    : _type{ Type::JSON_DOUBLE }, _value{ value } {}
...
Json::Json(const Json& value) { copy(value); }
...
Json::~Json() { _value = nullptr; _type = Type::JSON_NULL; }
...
void Json::copy(const Json& other) {
    ...
    case Type::JSON_STRING:
        _value = std::make_unique<string>(*std::get<str_ptr>(other._value).get());
        break;
    case Type::JSON_ARRAY:
        _value = std::make_unique<array>(*std::get<arr_ptr>(other._value).get());
        break;
    case Type::JSON_OBJECT:
        _value = std::make_unique<object>(*std::get<obj_ptr>(other._value).get());
        break;
    default:
        break;
    }
...
void Json::operator=(std::nullptr_t value) { clear(); copy(Json{ value }); }

void Json::operator=(const bool value) { clear(); copy(Json{ value }); }

void Json::operator=(const int value) { clear(); copy(Json{ value }); }

void Json::operator=(const double value) { clear(); copy(Json{ value }); }

void Json::operator=(const char* value) { clear(); copy(Json{ value }); }

void Json::operator=(const string& value) { clear(); copy(Json{ value }); }

void Json::operator=(const array& value) { clear(); copy(Json{ value }); }

void Json::operator=(const object& value) { clear(); copy(Json{ value }); }
...
bool Json::equal(const Json& other) const
{
    ...
    switch (_type) {
    case Type::JSON_STRING:
        return *(std::get<str_ptr>(_value)) == *(std::get<str_ptr>(other._value));
    case Type::JSON_ARRAY:
        return *(std::get<arr_ptr>(_value)) == *(std::get<arr_ptr>(other._value));
    case Type::JSON_OBJECT:
        return *(std::get<obj_ptr>(_value)) == *(std::get<obj_ptr>(other._value));
    default:
...
bool Json::operator==(const Json& other) const { return equal(other); }

bool Json::operator!=(const Json& other) const { return !equal(other); }

Type Json::type() const { return _type; }
...
void Json::insert(std::pair<const char*, Json> k_v)
{
    if (_type != Type::JSON_STRING)
    {
        throw std::logic_error("insert(std::pair<const char*, Json> k_v) => type error");
    }
...
void Json::erase(const std::size_t index)
{
    ...
    std::get<arr_ptr>(_value)->erase(std::get<arr_ptr>(_value)->begin() + index);
}
